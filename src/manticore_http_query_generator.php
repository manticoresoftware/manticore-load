...
/**
 * Iterates over Manticore HTTP cache file: one JSON-encoded batch per line.
 * Implements Countable and IteratorAggregate for compatibility with ProgressDisplay and foreach.
 */
class ManticoreHttpCacheFileBatches implements IteratorAggregate, Countable {
    private $path;
...
/**
 * Generates Manticore HTTP request bodies (/bulk NDJSON or /search JSON) from JSON templates
 ...
 */
class ManticoreHttpQueryGenerator {
    private $config;
    private $load_infos = [];
     ...
     * @return array|ManticoreHttpCacheFileBatches Iterable of batch strings (bulk NDJSON or search JSON)
     */
    ...
    private function loadQueriesFromCache() {
        if ($this->cache_from_disk) {
            return new ManticoreHttpCacheFileBatches($this->cache_file_name);
        }
    ...
    /**
     * Parse JSON template: find placeholders and determine if document (bulk) or search.
     */
    private function parseLoadCommand($command) {
        ...
        $is_index = !preg_match('/"query"\s*:/', $command);
        return [
            'command' => $command,
            'patterns' => $patterns,
    ...
    /**
     * Build Manticore /bulk NDJSON: one insert action per document.
     * Document "id" (if present) is lifted next to table; remaining fields go under "doc".
     */
    private function formatBulk(array $docs, $indexName = null) {
        if ($indexName === null || $indexName === '') {
            throw new Exception("Table name is required for bulk inserts in --http mode");
        }
        $lines = [];
        foreach ($docs as $doc) {
            $decoded = is_array($doc) ? $doc : json_decode($doc, true);
            if (!is_array($decoded)) {
                throw new Exception("Invalid document JSON for bulk: " . substr((string)$doc, 0, 200));
            }
            $id = null;
            ...
            $action = ['insert' => ['table' => $indexName]];
            if ($id !== null) {
                $action['insert']['id'] = $id;
            }
            $action['insert']['doc'] = $decoded;
            $lines[] = json_encode($action, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
        return implode("\n", $lines) . "\n";
    }

    /**
     * Whether the workload is document bulk rather than search.
     */
